...
class LinkedInAccountForm(forms.ModelForm):
    ...
    client_id = forms.CharField(
        label=_("Client ID"),
        required=False,
        help_text=_("From LinkedIn Developer Portal → Auth tab → Application credentials"),
    )
    ...
    def clean(self):
        ...
        if (not access_token or access_token == "••••••••") and self.instance and self.instance.pk:
        ...
        if auth_code and client_id and client_secret:
            ...
            raise forms.ValidationError(
                _("Client ID and Client Secret are required when using Authorization Code.")
            )
